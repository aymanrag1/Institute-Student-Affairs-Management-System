<?php
/**
 * Portal – Boss Man Daily Study Report / لوحة تحكم الحكمدار – تقرير المتابعة الدراسية اليومي
...
<div class="rsyi-portal rsyi-bm-portal" dir="rtl" style="font-family:sans-serif; max-width:960px; margin:0 auto;">
    ...
    <!-- Daily Report Tab / تبويب التقرير اليومي -->
    <div id="rsyi-bm-tab-daily">
        <div style="background:#fff; border:1px solid #dee2e6; border-radius:10px; padding:20px; margin-bottom:20px;">
            ...
            <div style="text-align:center; padding:30px; color:#999;">
                ...
                <p>لا يوجد طلاب نشطون في دفعتك حالياً. / No active students in your cohort.</p>
            </div>
            <?php else : ?>
            ...
            <table style="width:100%; border-collapse:collapse;" id="bm-students-table">
                <thead>
                    <tr style="background:#f8f9fa; border-bottom:2px solid #dee2e6;">
                        ...
                        <th style="padding:10px 14px; text-align:right; font-size:13px;">اسم الطالب / Student Name</th>
                        <th style="padding:10px 14px; text-align:right; font-size:13px; min-width:200px;">الكورس / Course</th>
                        <th style="padding:10px 14px; text-align:right; font-size:13px;">ملاحظات / Notes</th>
                    </tr>
                </thead>
                <tbody>
...
jQuery(function($){
    ...
    // ── Load existing data for a date / تحميل بيانات تاريخ محدد ─────────────────
    ...
    $('#bm-save-btn').on('click', function(){
        ...
        $.post(ajaxUrl, {
            ...
            $btn.prop('disabled', false).html('💾 حفظ التقرير / Save Report');
            showNotice('❌ فشل الاتصال. حاول مجدداً. / Connection failed. Please retry.', 'error');
        });
    });
    ...
    $('#hist-load-btn').on('click', function(){
        ...
        $.get(ajaxUrl, {
            ...
            var html = '<table style="width:100%; border-collapse:collapse;">' +
                ...
                '<th style="padding:8px 12px; text-align:right;">الطالب / Student</th>' +
                '<th style="padding:8px 12px; text-align:right;">الكورس / Course</th>' +
                '<th style="padding:8px 12px; text-align:right;">ملاحظات / Notes</th>' +
                '</tr></thead><tbody>';
            $.each(res.data.rows, function(i, r){
                html += '<tr style="border-bottom:1px solid #f0f0f0;">' +
                    ...
                    (r.course_name_ar
                        ? '<span style="background:#e3f2fd;color:#0073aa;padding:2px 10px;border-radius:20px;font-size:12px;">' + r.course_name_ar + (r.course_name_en ? ' / ' + r.course_name_en : '') + '</span>'
